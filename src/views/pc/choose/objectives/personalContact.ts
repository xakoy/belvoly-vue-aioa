...
export const personalContactObjective: ChooseObjective<{
    ...
    getSearchUrl: function(config, searchText) {
        return `${rootConfig.apiHost}/private/contacts/orgview/search?pageSize=10000&currentPage=1&categoryType=4&name=${encodeURIComponent(searchText)}&isThrough=true&groupId=0`
    },
    getUrl: function(config, url, category, type) {
        if (type === 'category') {
            return `${rootConfig.apiHost}/private/contacts/group/personalTree`
        } else {
            return `${rootConfig.apiHost}/private/contacts/orgview/search?pageSize=10000&currentPage=1&categoryType=4&name=&isThrough=false&groupId=` + category
        }
    }
}
